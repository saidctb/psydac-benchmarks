...
number_of_mpi_procs = [1,2,4]
number_of_threads = 1

timmings_bi_assembly    = np.zeros((len(problems), len(ncells),len(degrees), len(number_of_mpi_procs)))
timmings_dot_p_assembly = np.zeros((len(problems), len(ncells),len(degrees), len(number_of_mpi_procs)))
scaling_bi_assembly     = timmings_bi_assembly.copy()
scaling_dot_p_assembly  = timmings_dot_p_assembly.copy()
        ...
        for i3,d in enumerate(degrees):
            for i4, mpi_p in enumerate(number_of_mpi_procs):
                ...
                try:
                    ...
                    T = T.item()
                    timmings_bi_assembly[i1,i2,i3,i4] = T['dot_product_time']
                    timmings_dot_p_assembly[i1,i2,i3,i4] = T['bilinear_form_assembly_time']
                except:
                    timmings_bi_assembly[i1,i2,i3,i4] = np.nan
                    timmings_dot_p_assembly[i1,i2,i3,i4] = np.nan
            ...
            scaling_dot_p_assembly[i1,i2,i3,:] = timmings_dot_p_assembly[i1,i2,i3,:]/timmings_dot_p_assembly[i1,i2,i3,k[0]]
